...
flowchart LR
    ...
    B -->|"/api/level/** or /api/threshold/** or /api/leaderboard/** or /api/notifications/** or /api/ranks/**"| D[gamificationRoute]
...
public RouterFunction<ServerResponse> gamificationRoute(RateLimitProperties props) {
    ...
    return route("gamification")
            .route(path("/api/level/**").or(path("/api/threshold/**")).or(path("/api/leaderboard/**"))
                    .or(path("/api/notifications/**")).or(path("/api/ranks/**")), http())
